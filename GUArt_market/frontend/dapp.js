...
const dApp = {
  ...
  registerLand: async function() {
    ...
    try {
      ...
      await this.artContract.methods.registerLand(reference_uri).send({from: this.accounts[0]}).on("receipt", async (receipt) => {
        M.toast({ html: "Transaction Mined! Refreshing UI..." });
  ...
  main: async function() {
    ...
    this.marketJson = await (await fetch("./GUArtMarket.json")).json();
    this.auctionJson = await (await fetch("./GUArtAuction.json")).json();
