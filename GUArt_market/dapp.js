import marketJson from "./GUArtMarket.json";
import auctionJson from "./GUArtAuction.json";

const Web3 = require("web3");

const contractAddress = ""; // insert GUArtMarket contract address here
...
const dApp = {
  ...
  init: async function() {
    ...
    this.guartContract = new window.web3.eth.Contract(
      marketJson,
      this.contractAddress,
      { defaultAccount: this.accounts[0] }
    );
    this.isAdmin = this.accounts[0] == await this.guartContract.methods.owner().call();
    console.log("Contract object", this.guartContract);
  },
  collectVars: async function() {
    // get land tokens
    this.tokens = [];
    this.totalSupply = await this.guartContract.methods.totalSupply().call();
    for (let i = 0; i < this.totalSupply; i++) {
      this.tokens.push({
        tokenId: i,
        highestBid: Number(await this.guartContract.methods.highestBid(i).call()),
        auctionEnded: Boolean(await this.guartContract.methods.auctionEnded(i).call()),
        pendingReturn: Number(await this.guartContract.methods.pendingReturn(i, this.accounts[0]).call()),
        auction: new window.web3.eth.Contract(
          auctionJson.abi,
          await this.guartContract.methods.getAuction(i).call(),
          { defaultAccount: this.accounts[0] }
        ),
        owner: await this.guartContract.methods.ownerOf(i).call(),
        ...JSON.parse(await this.guartContract.methods.tokenURI(i).call())
      });
    }
  },
  ...
  bid: async function(event) {
    ...
    await this.guartContract.methods.bid(tokenId).send({from: this.accounts[0], value: wei}, async () => {
      await this.updateUI();
    });
  },
  endAuction: async function(event) {
    const tokenId = $(event.target).attr("id");
    await this.guartContract.methods.endAuction(tokenId).send({from: this.accounts[0]}, async () => {
      await this.updateUI();
    });
  },
  ...
  registerLand: async function() {
    ...
    await this.guartContract.methods.registerLand(JSON.stringify(tokenURI)).send({from: this.accounts[0]}, async () => {
